...
def command_show(calendar):
    ...
    for each in keys_calender:
        i = 0
        data += "\n    "+each+":"
        for value in calendar.get(each):
            data += "\n        "+str(i)+": "+value
            i += 1
    return data
    ...
    if is_calendar_date(date):
        if date in calendar.keys():
            if entry_number < len(calendar[date]):
                remove_data = calendar[date]
                calendar[date].remove(remove_data[entry_number])
            ...
            return str(date)+" is not a date in the calendar"
    else:
        return str(date)+" is not a valid date"
        ...
        if file.mode == 'r':
            ...
            while not contents == "":
                ...
                for i in range(0, len(events)):
                    if not events[i] == "":
                        command_add(date, events[i], calender)
                contents = file.readline()
        file.close()
        return calender
    else:
...
def is_command(command):
    ...
    '''
    valid_data = ["add", "delete", "quit", "help", "show"]
    if command in valid_data:
        return True
    else:
        return False
...
def is_calendar_date(date):
    ...
    #2017-100-21
    if len(date) == 10:
        if not (date[4] == '-' or date[7] == '-'):
            return False
        else:
            for i in range(0, len(date)):
                if is_natural_number(date[i]) or date[i] == '-':
...
def is_natural_number(str):
    ...
    for i in range(0, len(str)):
        if not (ord(str[i]) in range(48, 57)):
            return False
    return True
...
def parse_command(line):
    ...
    command_detail = ['add', 'delete']
    small_command = ['help', 'show', 'quit']
    if is_command(storage[0]):
        if storage[0] in small_command:
            if len(storage) > 1:
                return ['error', storage[0]]
            return [storage[0]]
        if storage[0] in command_detail[0]:
            if len(storage) == 3:
                if is_calendar_date(storage[1]):
                    return [storage[0], storage[1], storage[2]]
                else:
                    return ['error', 'not a valid calendar date']
            else:
                return ['error', 'add DATE DETAILS']
        elif storage[0] in command_detail[1]:
                if len(storage) == 3:
                    if not is_calendar_date(storage[1]):
                        return ['error', 'not a valid calendar date']
                    elif not is_natural_number(storage[2]):
